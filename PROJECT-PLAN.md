...
5. Per-word font-size fix: clicking the `<input type=number>` size control fired the contentEditable blur with `relatedTarget=null`, committing+exiting edit so size hit the whole zone. `handleBlur` now defers on null relatedTarget and checks `document.activeElement`.
